...
self.addEventListener('fetch', (event) => {
  ...
  // Network-first for everything else (SPA routes)
  event.respondWith(
    fetch(request)
      .then((response) => {
...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || 'DutyDesk', {
      body: data.message || '',
      icon: '/icon-192.png',
      badge: '/icon-192.png',
      tag: data.tag || 'dutydesk-notification',
      data: { url: data.url || '/' },
    })
  );
});
